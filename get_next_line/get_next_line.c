...
char	*get_next_line(int fd)
{
	...
	{
		temp = next_lines;
		// if (test_line_check > 5) printf("temp is assigned to next_lines, i.e. to \"%s\"\n", temp);
	}
	else
	{
	...
	while (((end_of_line = ft_end_of_line(temp)) == 0) && ft_strlen(temp)) // if no '\n' in buf or next_lines, add contents to line and read further
	{
		...
		// if (test_line_check > 5) printf("in the while loop, line has now been assigned to:\n\"%s\"\n\n", line);
		free(end_of_line);
 		// if (test_line_check > 5) printf("   and temp is \"%s\"\n\n", temp);
		temp = ft_fill_zero(buf, BUFFER_SIZE); // implicit malloc, freed by free(buf) at the end of the function
		if (read(fd, (temp = buf), BUFFER_SIZE) < 5) end_reached = 1;
		// if (test_line_check > 5) printf("After ft_fill_zero and reading again, temp is \"%s\" at address %p\n*********************************************************\n", temp, temp);
	}
