...
/*   Updated: 2024/04/15 17:39:16 by francis          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_put_point_list(t_env *env, t_list *anchor, float zoom)
{
	...
	if (env->point_list)
	{
		while (env->point_list->next)
		{
			...
			if (((int *) env->point_list->content)[1] < y_min)
			{
	...
	ft_printf("y_min: %d, y_max: %d \n", (int)y_min, (int)y_max);
	// defining zoom from maximum and minimum values
	while  (((x_max - x_min) * zoom > WINDOW_WIDTH) || ((y_max - y_min) * zoom > WINDOW_HEIGHT))
		zoom -= 0.1;
