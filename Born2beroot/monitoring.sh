...
SUDOS=$(cat /var/log/sudo/sudo.log | grep "COMMAND" | wc -l)
SUDO=$(journalctl _COMM=sudo -q) > sudocount.tmp | awk '{while (match($0, /COMMAND/)) {count++; $0=substr($0, RSTART+RLENGTH)}} END {print count}' sudocount
#This yields an error "-bash: Dec: command not found"

wall "Broadcast message from $(whoami)@$(hostname) ($(date)):
