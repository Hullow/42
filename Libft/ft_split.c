...
char	**ft_split(char const *s, char c)
{
	...
	while (i < count)
	{
		...
		copy = ft_strtrim(ft_substr(copy, length, ft_strlen(copy)), character); // trims the string by one word and prepares the string for the next loop
		printf("copy: %s\n", copy);
		// free (split[i]);
		i++;
	}
