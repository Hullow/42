...
/*   Updated: 2024/05/27 20:25:52 by fallan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_calculate_cost(t_stack_list *a_element, t_stacks *full_stack, t_cost *cost)
{
	...
	printf("size: a %d, b %d, optimal_position: %d\n", full_stack->size_a, full_stack->size_b, optimal_position);
	...
	if (i == 0)
	{
		...
		printf("min_cost(%d) index selected: %d: (%d RR, %d RA, %d RB)\n", min_cost, i, cost->xRR, cost->xRA, cost->xRB);
	}
	else if (i == 1)
	{
		cost->xRR = 0;
		...
		printf("min_cost(%d) index selected: %d: (%d RRR, %d RRA, %d RRB)\n", min_cost, i, cost->xRR, cost->xRA, cost->xRB);
	}
	else if (i == 2)
	{
		cost->xRR = 0;
		...
		cost->xRRR = 0;
		cost->xRRA = 0;
		printf("min_cost(%d) index selected: %d: (%d RA, %d RRB)\n", min_cost, i, cost->xRA, cost->xRRB);
	}
	else if (i == 3)
	{
		cost->xRR = 0;
		...
		cost->xRRR = 0;
		cost->xRRB = 0;
		printf("min_cost(%d) index selected: %d: (%d RB, %d RRA)\n", min_cost, i, cost->xRB, cost->xRRA);
	}
	cost->total = cost->xRA + cost->xRB + cost->xRRA + cost->xRRB + cost->xRR + cost->xRRR;
	printf("total cost: %d (from: %d (xRA) + %d (xRA_aggregated) + %d (xRRA) + %d (xRRA_aggregated) + %d (xRB) \
